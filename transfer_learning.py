...
INPUT_DIR2 = '/home/ahmadreza.nazari/test/image/'
TARGET_DIR2 = '/home/ahmadreza.nazari/test/label/'
...
print(f'Real data: Found {len(input_paths2)} images.')

target_paths2 = sorted(
    [
...
print(f'Real data: Found {len(target_paths2)} masks.')

if (len(input_paths2) != len(target_paths2)):
    raise Exception('dataset errror')

indexes = np.arange(len(input_paths2), dtype=int)
np.random.shuffle(indexes)

input_paths2 = [input_paths2[i] for i in indexes]
target_paths2 = [target_paths2[i] for i in indexes]

print('Real data: Paths: ')
for input_path, target_path in zip(input_paths2[:5], target_paths2[:5]):
    print(input_path, '|', target_path)
...
class DataSequence(tf.keras.utils.Sequence):
    ...
    def __init__(self, in_paths, out_paths, batch_size=BATCH_SIZE, img_size=IMG_SIZE, n_channels=N_CHANNELS, n_classes=num_classes):
    ...
    def __getitem__(self, idx):
        ...
        x = np.zeros((self.batch_size, ) + self.img_size +
                     (self.n_channels, ), dtype='float32')
        for j, path in enumerate(batch_input_paths):
            img = tf.keras.preprocessing.image.load_img(
                path, color_mode='rgb', target_size=self.img_size)
...
val_samples = int(0.15 * len(input_paths2))
test_samples = int(0.15 * len(input_paths2))
train_samples = len(input_paths2) - val_samples - test_samples

train_input_paths2 = input_paths2[:train_samples]
train_target_paths2 = target_paths2[:train_samples]

val_input_paths2 = input_paths2[train_samples:train_samples + val_samples]
val_target_paths2 = target_paths2[train_samples:train_samples + val_samples]

test_input_paths2 = input_paths2[train_samples +
                               val_samples: train_samples + val_samples + test_samples]
test_target_paths2 = target_paths2[train_samples +
                                 val_samples:train_samples + val_samples + test_samples]

train_gen2 = DataSequence(train_input_paths2, train_target_paths2)
val_gen2 = DataSequence(val_input_paths2, val_target_paths2)
test_gen2 = DataSequence(test_input_paths2, test_target_paths2)

print('Real data: train_samples', train_samples)
print('Real data: val_samples', val_samples)
...
print('Evaluating with real data...')
metrics = model.evaluate(test_gen2)
for i, metric in enumerate(metrics):
    print(f'Metric {i}:', metric)
...
plt.savefig('/home/ahmadreza.nazari/results/simulation-data.png')

plt.figure(figsize=(8, 20), dpi=300)
indexes = np.arange(len(test_input_paths2), dtype=int)
indexes = indexes[:plt_number]
for j, i in enumerate(indexes):
    img = plt.imread(test_input_paths2[i])
    plt.subplot(plt_number, 3, j*3+1)
    plt.imshow(img/img.max())
    plt.axis('off')
    plt.title('Image')
    plt.subplot(plt_number, 3, j*3+2)
    predicted_mask = np.argmax(test_preds2[i], axis=-1)
    ...
    plt.subplot(plt_number, 3, j*3+3)
    actual_mask = tf.keras.preprocessing.image.load_img(
        test_target_paths2[i], color_mode='rgb', target_size=IMG_SIZE)
    actual_mask = tf.keras.preprocessing.image.img_to_array(actual_mask)
    plt.imshow(actual_mask/actual_mask.max())
    plt.axis('off')
    plt.title('Actual Mask')

plt.savefig('/home/ahmadreza.nazari/results/real-data.png')


# %% Freeze the base, train the classifier

tf.keras.backend.clear_session()
model = get_unet_mod(base_trainable=False)
model.summary()

model.save_weights('/home/ahmadreza.nazari/unet_seg_v11_modified.h5')
model_name = 'unet_seg_v11_modified.h5'
...
model.load_weights('unet_seg_v11.h5')
epochs = 10
history_modified = model.fit(train_gen2, epochs=epochs, validation_data=val_gen2, callbacks=callbacks)

# %% Unfreeze the base, fine tune the model

tf.keras.backend.clear_session()
model = get_unet_mod(learning_rate=1e-6)
model.summary()
model.save_weights('/home/ahmadreza.nazari/unet_seg_v11_modified2.h5')
model_name = 'unet_seg_v11_modified2.h5'

model.load_weights('unet_seg_v11_modified.h5')
epochs = 1
history_modified2 = model.fit(train_gen2, epochs=epochs, validation_data=val_gen2, callbacks=callbacks)
...
tf.keras.backend.clear_session()
model = get_unet_mod()
model.load_weights('unet_seg_v11_modified2.h5')

print('Testing with simulation data...')
metrics = model.evaluate(test_gen)
print('Dice Coef:', metrics[0])
print('Accuracy:', metrics[1])

print('Testing with real data...')
metrics2 = model.evaluate(test_gen2)
...
plt.savefig('/home/ahmadreza.nazari/results/simulation-data-modified.png')
...
plt.savefig('/home/ahmadreza.nazari/results/real-data-modified.png')
